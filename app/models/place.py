...
class Place(db.Model):
    ...
    area = db.Column(db.String(20), nullable=False)      # 政大周邊 / 信義 / 中山
    category = db.Column(db.String(30), nullable=False)  # 餐廳 / 景點 / 活動 / 咖啡廳
    description = db.Column(db.Text)

    # 條件篩選欄位
    indoor = db.Column(db.Boolean, nullable=False)        # True=室內, False=室外
    activity_type = db.Column(db.String(10), nullable=False)  # 靜態 / 動態 / 兩者
    budget_min = db.Column(db.Integer, default=0)         # 每人最低消費（元）
    budget_max = db.Column(db.Integer, default=0)         # 每人最高消費（元）

    # 適合時段（存成 JSON 字串，例如 '["早上","下午"]'）
    suitable_time = db.Column(db.Text, default='["早上","下午","晚上"]')

    # 適合旅伴（存成 JSON 字串，例如 '["情侶","朋友"]'）
